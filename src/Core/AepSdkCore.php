...
namespace Hiworld\LaravelIot\Core;
...
class AepSdkCore
{
    ...
    public static function init(array $config)
    {
        if (empty($config['base_url'])) {
            throw new \InvalidArgumentException("Base URL must be provided in config");
        }

        self::$baseUrl = rtrim($config['base_url'], '/');
        self::$timeUrl = $config['time_url'] ?? self::$baseUrl . '/echo';
    }
